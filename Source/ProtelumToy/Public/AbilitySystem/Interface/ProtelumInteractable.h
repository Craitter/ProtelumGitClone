...
#include "ProtelumInteractable.generated.h"
...
class UProtelumInteractable : public UInterface
{
	GENERATED_BODY()
};
 ...
 * 
 */
class PROTELUMTOY_API IProtelumInteractable
{
	GENERATED_BODY()
...
public:
	...
	* ScopedPredictionKey.
	*
	* Player revives use PreInteract() to trigger an ability that plays an animation that lasts the same duration as
...
protected:
	TMap<TWeakObjectPtr<UPrimitiveComponent>, TArray<TWeakObjectPtr<AActor>>> Interacters;
};
